...
# # Filter DataFrame based on 'model' and group by 'snr_db'
# grouped1 = df[df['model'] == 'airtnn'].groupby('model.backbone.snr_db')['test/acc'].agg(['mean', 'std'])
# grouped2 = df[df['model'] == 'airgnn'].groupby('model.backbone.snr_db')['test/acc'].agg(['mean', 'std'])
...
for model in models:
    ...
    for idx, row in hyperparameters.iterrows():
        ...
        # Group by 'snr_db' and compute mean and std
        grouped = df_hyper.groupby('model.backbone.snr_db')['test/acc'].agg(['mean', 'std'])
